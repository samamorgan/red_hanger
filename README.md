...
## Overview
Ensure coat hangers are used for their intended purpose
...
- `/red_hanger` - the Django project folder
...
- Retrieve Media assets from AWS S3:
    - Set up and configure the AWS CLI: see <https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-install.html> for instructions
        - be sure to set up your AWS config with your red_hanger credentials: instructions here <https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html>
    - make sure a directory exists with the path [project root]/red_hanger/media
    - from the project root run: `aws s3 cp s3://red_hanger-dev ./red_hanger/media --recursive` replacing `red_hanger-dev` with `red_hanger-prod` or a different s3 bucket name as needed
